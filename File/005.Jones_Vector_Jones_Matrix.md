...
\therefore S' _ 1&=\vert \alpha \vert^2+\vert \beta \vert^2
\end{aligned}
$$

$$
\begin{aligned}
S' _ 2&=2Re\bigl(-\bigl(Re(\alpha)\cos{\delta}-Im(\alpha)\sin{\delta}+i(Re(\alpha)\sin{\delta}+Im(\alpha)\cos{\delta})\bigr)(Re(\beta)-iIm(\beta))\bigr)
\end{aligned}
$$